...
class GPT(nn.Module):
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        tk_emb = self.token_embedding(context)
        B, T, D = tk_emb.shape
        pos_emb = self.pos_embedding(torch.arange(T))
        tot_emb = tk_emb + pos_emb

        block_out = self.transformers(tot_emb)
        f_norm = self.final_norm(block_out)
        out = self.vocab_proj(f_norm)
        return torch.round((nn.functional.softmax(out, dim=-1)), decimals=4)
