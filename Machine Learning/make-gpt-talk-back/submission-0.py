...
class Solution:
    def generate(self, model, new_chars: int, context: TensorType[int], context_length: int, int_to_char: dict) -> str:
        ...
        for i in range(new_chars):
            ...
            prev_time_step = prediction[:,-1,:]
            probabilities = nn.functional.softmax(prev_time_step, dim=-1)
            next_tk = torch.multinomial(probabilities, 1, generator=generator)
            ...
            context = torch.cat((context, next_tk), dim=-1)
            ret.append(int_to_char[next_tk.item()])
