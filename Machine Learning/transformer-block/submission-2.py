...
class TransformerBlock(nn.Module):
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        att_norm = self.norm1(embedded)
        att = self.mhsa.forward(att_norm)
        att_out = embedded + att
        ff_norm = self.norm2(att_out)
        ff = self.ff.forward(ff_norm)
        ff_out = att_out + ff

        return torch.round(ff_out, decimals=4)
